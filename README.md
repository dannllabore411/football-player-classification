...
2. **Feature Engineering & Standardization**  
   Creates metrics to derive average position (verticality & width) from event data. Applies z-score normalization per position group to enable fair comparisons across stats.   
   *feature engineering, standardization*
3. **Supervised Position Group Prediction (`PosGrp`)**  
   Trains an XGBoost classifier to assign each player to one of 7 position groups (GK, CB, FB, DM, AM, WF, CF) based only on raw stats using a training set (posgrp_labeled_raw.csv).  **TO-DO: Identify which variables vary by PosGrp**
   *supervised classification, model training, label encoding*
4. **Unsupervised Role Clustering**  
   Uses KMeans to cluster players into 3 roles within each position group (e.g., CB-1, CB-2, CB-3).  **TO-DO: Identify which variables vary by cluster**
...
7. **Export Results & Trained Models**  
   Saves player classifications, visual plots, cluster examples, and the trained PosGrp model for reuse.  **TO-DO: Output - show charts based on critical variables, show PosGrp and Role in terms of probability**
